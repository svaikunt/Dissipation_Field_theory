...
\end{equation}
formally analogue to the relation in~\eqref{eq:approx} for driven tracers up to an overall factor of $2$. Conducting the same analysis of density correlations as for driven tracers, ${\cal I}$ exhibits again a scaling with ${\rm Pe}^2$, as reported in Fig.~\ref{fig:fig2_ac}. Besides, we show that $\dot w$ and $\int{\cal I}({\bf r}){\rm d}{\bf r}$ are in good agreement when now introducing the factor $\rho_0/(2\gamma)$. It demonstrates that dissipation is also accessible from the nonequilibrium deviation of pair correlation in fully driven liquids.
