...
\begin{equation}\label{eq:balance}
	\begin{aligned}
		&\gamma\dot w = 2\bar\rho_0 \int g({\bf r}) \big[(\nabla v({\bf r}))^2 - T \nabla^2 v({\bf r})\big] \dd{\bf r} ,
		\\
		&\, + \bar\rho_0^2 \iint\big[ g_{3a}({\bf r}, {\bf r}') + g_{3b}({\bf r}, {\bf r}')\big]  \big[\nabla v({\bf r})\big] \cdot \big[\nabla v({\bf r}')\big] \dd{\bf r} \dd{\bf r}' ,
	\end{aligned}
\end{equation}
where $\bar\rho_0 = \rho_0(1-p)$ is defined in terms of the total density $\rho_0$ and the fraction of driven tracers $p$, and
\begin{equation}
	\begin{aligned}
...
To evaluate further the change in liquid structure induced by dissipation, we measure the deviation from equilibrium pair correlations $g-g_{\rm eq}$ due to the driving forces (left column in Fig.~\ref{fig:fig2}). In particular, inspired by the two-body contribution in the power balance~\eqref{eq:balance}, we focus on the observable ${\cal I} = \big[(\nabla v)^2-T\nabla^2v\big](g - g_{\rm eq})$. At a given $\tau/\tau_{\rm r}$, scaling ${\cal I}$ by ${\rm Pe}^2$ reveals that all curves almost collapse into a master curve for our numerical range ${\rm Pe}\in[12,36]$, as reported in the middle column of Fig.~\ref{fig:fig2}. In practice, particles overlap more for a stronger drive, so that $g$ departs from zero at smaller interparticle distance. To correct for this, we introduce a shift of the curves ${\cal I}(|{\bf r}|)$ as $|{\bf r}|\to |{\bf r}|+a$, where $a(\rm Pe)$ is a fitting parameter. Given that the rate of work also scales like ${\rm Pe}^2$, it suggests the existence of an underlying relation between $\int{\cal I}({\bf r}){\rm d}{\bf r}$ and $\dot w$. In practice, introducing a factor $\rho_0/\gamma$ provides a satisfactory agreement between them, as shown in the right column of Fig.~\ref{fig:fig2}. This empirical relation demonstrates that, in the dilute limit, dissipation can actually be directly estimated by comparing driven and equilibrium pair correlations for both deterministic and active drives. 

This empirical relation, in combination with ~\eqref{eq:balance}, suggests the following connection between two and three body correlations in our non-equilibrium system
\begin{equation}
    \label{eq:empiricalbalance1}
    \begin{aligned}
    &\rho^2 \int \int\ d{\bf r} d{\bf r}^\prime g_3({\bf r},{\bf r}^\prime)\left[\nabla u({\bf r})\cdot \nabla u({\bf r}^\prime)\right]\\
    &\quad =-\rho_0 \int d{\bf r}\frac{1}{2}\left[g({\bf r}+g_{\rm eq}({\bf r})\right]\left[\nabla u({\bf r})^2-T\nabla^2 u({\bf r})\right]\,.
    \end{aligned}
\end{equation}
We note the structure of the empirical connection in \eqref{eq:empiricalbalance1} is reminiscent of the connection between two and three body correlations at equilibrium as provided by the YBG hierarchy~\cite{Hansen2013} with the equilibrium pair correlation function replaced by correlation function averaged over the equilibrium and non-equilibrium steady states. Finally, we note that this connection can be further rewritten in a compact form that relates the deviations of two and three body correlations from their equilibrium values, 
\begin{equation}
    \label{eq:empiricalbalance2}
    \begin{aligned}
    &\rho^2 \int \int\ d{\bf r} d{\bf r}^\prime \left[g_3({\bf r},{\bf r}^\prime)-g_{3,\rm eq}({\bf r},{\bf r}^\prime)\right]\left[\nabla u({\bf r})\cdot \nabla u({\bf r}^\prime)\right]\\
    &\quad =-\rho_0 \int d{\bf r}\frac{1}{2}\left[g({\bf r}-g_{\rm eq}({\bf r})\right]\left[\nabla u({\bf r})^2-T\nabla^2 u({\bf r})\right]\,.
    \end{aligned}
\end{equation}
...
\begin{equation}\label{eq:balance_b}
	\begin{aligned}
		\gamma\dot w &= \rho_0 \int g({\bf r}) \big[(\nabla v({\bf r}))^2 - T \nabla^2 v({\bf r})\big] \dd{\bf r} ,
		\\
		&\quad + \rho_0^2 \iint g_3({\bf r}, {\bf r}') \big[\nabla v({\bf r})\big] \cdot \big[\nabla v({\bf r}')\big] \dd{\bf r} \dd{\bf r}' ,
	\end{aligned}
\end{equation}
...
\begin{equation}
	\gamma\dot w = \sum_i \big[ \big\langle{\bf F}_i^2\big\rangle + T \big\langle\nabla_i\cdot{\bf F}_i\big\rangle \big] ,
\end{equation}
formally analogue to the relation in~\eqref{eq:approx} for driven tracers up to an overall factor of $2$. Conducting the same analysis of density correlations as for driven tracers, ${\cal I}$ exhibits again a scaling with ${\rm Pe}^2$, as reported in Fig.~\ref{fig:fig2_ac}. Besides, we show that $\dot w$ and $\int{\cal I}({\bf r}){\rm d}{\bf r}$ are in good agreement when now introducing the factor $\rho_0/(2\gamma)$. We note that this empirical connection with the modified factor of $\rho_0/(2\gamma)$ is still consistent with the non-equilibrium connection between two and three body correlations proposed in \eqref{eq:empiricalbalance1}, \eqref{eq:empiricalbalance2}. These results demonstrate that dissipation is also accessible from the nonequilibrium deviation of pair correlation even in fully driven (or active) liquids.
...
\begin{equation}\label{eq:dyn_eq}
	\gamma\dot{\bf r}_i = - \nabla_i \sum_j v({\bf r}_i-{\bf r}_j) + {\boldsymbol\xi}_i ,
\end{equation}
where the statistics of the noise term ${\boldsymbol\xi}_i$ is the same as the one in~\eqref{eq:dyn}. The rate of work $\dot w$ defined in~\eqref{eq:work} is zero because of the absence of the driving force. In Sec.~\ref{sec:method}, to obtain a non-zero rate of energy flow through the system, we consider an explicit driving force ${\bf F}_{{\rm d},i}$ and we explore its effects on the structural properties of the liquid. In this section, using the framework of large deviation theory, we take an alternative approach where the dynamics is now conditioned by enforcing a required energy flow. Thus, exploring how the system adapts to this requirement provides a new insight, distinct from the one in Sec.~\ref{sec:method}, into the relation between dissipation, structural properties and organization in driven systems.


To this end, we focus on the subset of noise realizations that are effectively equivalent to dynamics with non-zero rate of work. Such dynamics are {\it atypical} since they correspond to realizations largely deviated from the most probable ones, thus opening the door to stabilizing structure and to promoting collective effects which are not {\it typically} observed. In particular, for this subset, the noise term in~\eqref{eq:dyn_eq} no longer has zero average, so that one can re-define it as ${\boldsymbol \xi}_i \to {\boldsymbol \xi}_i + {\bf F}_i$ in terms of an {\it auxiliary force} force ${\bf F}_i$~\cite{Jack2010, Chetrite2013}. Hence, it provides an explicit realization of forces that will ensure a non-zero energy flow rate.
