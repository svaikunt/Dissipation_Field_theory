...
\begin{equation}\label{eq:dyn_eq}
	\gamma\dot{\bf r}_i = - \nabla_i \sum_j v({\bf r}_i-{\bf r}_j) + {\boldsymbol\xi}_i ,
\end{equation}
where the statistics of the noise term ${\boldsymbol\xi}_i$ is the same as the one in~\eqref{eq:dyn}. At variance with Sec.~\ref{sec:method}, we aim to change the emerging collective behavior by means of a dynamical bias, instead of an external force, where trajectories are now driven by some atypical realizations of the noise. {\color{green} Formally, trajectories are biased by introducing an exponential weighting factor $\exp\big[\kappa\int_0^t{\cal E}(s){\rm d}s\big]$ in the dynamical path probability. Following the thermodynamic formalism, $\cal E$ and $\kappa$ respectively denote a given observable and its associated control parameter~\cite{Touchette2009}. The trajectories and configurations in the biased ensemble are then equivalent to rare events of the original dynamics~\eqref{eq:dyn_eq}, conditioned by atypical values of $\cal E$ which can be tuned at will by changing $\kappa$.
