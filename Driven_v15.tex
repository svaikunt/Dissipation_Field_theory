...
\textcolor{red}{Overall, the results of this Section illustrate how dissipation affects the transport and structural properties of driven liquids, measured in terms of diffusion coefficient and density fluctuations. This immediately suggests that internal transport and emerging structure can potentially be modified in a controlled manner by tuning the nonequilibrium forces which power the dissipation. These findings motivate a further question: namely can non-equilibrium driving forces be tuned so that they reliably stabilize configurations characteristic of microscopic interactions different from those specified by the atomistic force fields ?  We explore this in what follows using the framework of large deviation theory for the dissipation. The choice of the biasing function in the large deviation framework is informed by Eq.~\ref{eq:balance}. In particular, our analytical and numerical results provide intuition for how the effective interactions between constituents in a multicomponent system can be controllably renormalized using non-equilibrium forces. Such controlled renormalization can promote the nucleation of structures different from those characteristic of the equilibrium Boltzmann distribution and help guide self-assembly even far from equilibrium~\cite{Bisker2018}. These results further illustrate the interplay between energy consumption and organization in many body non-equilibrium settings
}

% Instead of anticipating which configurations are stabilized by external forces, a fruitful route is to first choose the microscopic interactions appropriate a target configuration, and then design the external protocol which effectively stabilizes this configuration. To this end, the challenge is to connect the particle trajectories driven by a protocol and the ones emerging spontaneously for modified interactions.



%{For example, imagine that we are interested in the pairwise interactions between the $i^{\rm th}$ and $j^{\rm th}$ particles. Using Eq.~\ref{eq:balance}, we construct a statistical quantity which characterizes the rate at which energy is pumped into or extracted out of the pairwise interactions. In a typical equilibrium ensemble, we expect these rates of energy flow to be zero. In the next section, instead of adding explicit driving forces to generate non-zero average energy flow rates (as described by Eq.~\ref{eq:balance}), we use the large deviation framework to bias noise realizations and sample trajectories so that this expectation deviates from zero on average.  Our main analytical and numerical results then show how the  effective interactions between the $i$th and $j$th particles in the biased ensembles can be renormalized in a controlled manner at the cost of the biased energy flows.}
...
\begin{equation}\label{eq:dyn_eq}
	\gamma\dot{\bf r}_i = - \nabla_i \sum_j v({\bf r}_i-{\bf r}_j) + {\boldsymbol\xi}_i ,
\end{equation}
where the statistics of the noise term ${\boldsymbol\xi}_i$ is the same as the one in~\eqref{eq:dyn}. At variance with Sec.~\ref{sec:method}, we aim to change the emerging collective behavior by means of a dynamical bias, instead of an external force, where trajectories are now driven by some atypical realizations of the noise. {\color{green} Formally, biasing trajectories amounts to introducing an exponential weighting factor $\exp\big[\kappa\int_0^t{\cal E}(s){\rm d}s\big]$ in the dynamical path probability. Following the thermodynamic formalism, $\cal E$ and $\kappa$ respectively denote a given observable and its associated control parameter~\cite{Touchette2009}. The trajectories and configurations in the biased ensemble are then equivalent to large deviations of the original dynamics~\eqref{eq:dyn_eq}, conditioned by atypical values of $\cal E$ which can be tuned at will by changing $\kappa$.


To explore the properties of the biased ensemble, we rely on the framework of large deviation theory~\cite{Chetrite2013, Jack2010}. In typical dynamics, the empirical average $(1/t)\int_{0}^{t} ds {\cal E}(s)$ takes a value close to the expectation $\langle \cal E \rangle$ at large time $t$, yet it can deviate from it in when trajectories are biased. In such a case, the noise term in~\eqref{eq:dyn_eq} no longer has zero average, so that it can be re-defined in terms of an {\it auxiliary force} ${\bf F}_i$ as ${\boldsymbol \xi}_i \to {\boldsymbol \xi}_i + {\bf F}_i$. Large deviation theory generally ensures the existence and uniqueness of ${\bf F}_i$~\cite{Jack2010,Chetrite2013}. In this Section, we investigate how to anticipate the dynamics of dissipation-biased ensemble by deriving the corresponding auxiliary force ${\bf F}_i$.
} 
...
\subsection{\textcolor{green}{Dynamical bias and external forces}}\label{sec:biasexternal}
...
To explore how this bias modifies interactions, {\color{green}we first employ a derivation different from the path integral approach in Sec.~\ref{sec:biasexternal}}. Based on the procedure in~\cite{Jack2010,Chetrite2013}, the auxiliary physical dynamics, which has the same statistical properties as in the biased ensemble, can be constructed by solving the eigenvalue equation 
...
\subsection{\textcolor{green}{Numerical sampling of biased structure}}
