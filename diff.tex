...
%DIF DEL v_rev_3.tex                Sun May 19 22:21:06 2019
%DIF ADD ActiveDriven_v_rev_4.tex   Sun May 19 22:21:06 2019
...
\end{equation}
We aim to explore connections between $D$ and dissipation, which is defined from stochastic thermodynamics as the power of the forces exerted by all tracers on solvent: ${\cal J} = \sum_i\langle\dot{\bf r}_i\cdot(\gamma\dot{\bf r}_i-{\boldsymbol\xi}_i)\rangle$, where $\cdot$ denotes a Stratonovich product~\cite{Sekimoto1998, Seifert2012}. \DIFaddbegin \DIFadd{Dissipation is directly related to entropy production, as a measure of irreversibility, both when the drive is deterministic~\mbox{%DIFAUXCMD
\cite{Sekimoto1998, Seifert2012} }%DIFAUXCMD
and when it is a correlated noise~\mbox{%DIFAUXCMD
\cite{Mandal2017, Seifert2018, Shankar2018, Bo2019}}%DIFAUXCMD
. }\DIFaddend Substituting the dynamics~\eqref{eq:dyn}, the dissipation coincides with the power of driving forces: ${\cal J} = \sum_{i\in\Omega}\langle\dot{\bf r}_i\cdot{\bf F}_{{\rm d},i}\rangle$, and it can be further separated into free-motion and interaction contributions as ${\cal J} = f^2/\gamma - \dot w$, where the {\it rate of work} reads
...
\begin{figure*}
	...
	\caption{\label{fig:fig2}
		...
		(Middle)~Deviation from equilibrium correlations ${\cal I} = \big[(\nabla v)^2-T\nabla^2v\big](g - g_{\rm eq})$ scaled by ${\rm Pe}^2$ as a function of $(r+a)/\sigma$ where $a({\rm Pe})$ is a fitting parameter. The data almost collapse into a master curve for a given $\tau/\tau_{\rm r}$.
		(Right)~Integrated deviation from equilibrium correlations $\rho_0\int{\cal I}({\bf r}){\rm d}{\bf r}/(\gamma T)$ and rate of work $\dot w/T$ as functions of $\rm Pe$: the deviation from equilibrium correlations provides a direct access to dissipation with only a small error.
...
\DIFaddend This empirical relation demonstrates that, in the dilute limit, dissipation can actually be directly estimated by comparing driven and equilibrium pair correlations for both deterministic and active drives. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This empirical relation, in combination with }\DIFdelend \DIFaddbegin \DIFadd{Comparing}\DIFaddend ~\eqref{eq:balance} \DIFdelbegin \DIFdel{, suggests the following connection between two and three body correlations
in our non-equilibrium system
}\DIFdelend \DIFaddbegin \DIFadd{and~}\eqref{eq:emp}\DIFadd{, we deduce the following integral relation between density correlations
}\DIFaddend \begin{equation}\DIFdelbegin %DIFDELCMD < \label{eq:empiricalbalance1}
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \label{eq:emp_b}
	\DIFaddend \begin{aligned}
		&\DIFdelbegin \DIFdel{\rho^2 }\DIFdelend \DIFaddbegin \DIFadd{-}\DIFaddend \int \DIFdelbegin \DIFdel{\int\ d}\DIFdelend \DIFaddbegin \big[ \DIFadd{g(}\DIFaddend {\bf r}\DIFdelbegin \DIFdel{d}%DIFDELCMD < {\bf %%%
\DIFdel{r}%DIFDELCMD < }%%%
\DIFdel{^\prime }\DIFdelend \DIFaddbegin \DIFadd{) + }\DIFaddend g\DIFdelbegin \DIFdel{_3}\DIFdelend \DIFaddbegin \DIFadd{_{\rm eq}}\DIFaddend ({\bf r}\DIFdelbegin \DIFdel{,}%DIFDELCMD < {\bf %%%
\DIFdel{r}%DIFDELCMD < }%%%
\DIFdel{^\prime}\DIFdelend ) \DIFdelbegin %DIFDELCMD < \left[%%%
\DIFdelend \DIFaddbegin \big] \big[\DIFadd{(}\DIFaddend \nabla \DIFdelbegin \DIFdel{u}\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ({\bf r})\DIFdelbegin \DIFdel{\cdot }\DIFdelend \DIFaddbegin \DIFadd{)^2 - T }\DIFaddend \nabla\DIFdelbegin \DIFdel{u}\DIFdelend \DIFaddbegin \DIFadd{^2 v}\DIFaddend ({\bf r}\DIFdelbegin \DIFdel{^\prime}\DIFdelend )\DIFdelbegin %DIFDELCMD < \right]%%%
\DIFdelend \DIFaddbegin \big] \DIFadd{\dd{\bf r}
		}\DIFaddend \\
		& \DIFdelbegin %DIFDELCMD < \quad %%%
\DIFdel{=-}\DIFdelend \DIFaddbegin \DIFadd{\simeq }\DIFaddend \rho_0 \DIFdelbegin \DIFdel{\int d}\DIFdelend \DIFaddbegin \iint\big[ \DIFadd{g_{3a}(}\DIFaddend {\bf r}\DIFdelbegin \DIFdel{\frac{1}{2}}%DIFDELCMD < \left[%%%
\DIFdel{g(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend {\bf r}\DIFaddbegin \DIFadd{') }\DIFaddend + g\DIFdelbegin \DIFdel{_{\rm eq}}\DIFdelend \DIFaddbegin \DIFadd{_{3b}}\DIFaddend ({\bf r}\DIFaddbegin \DIFadd{, }{\bf \DIFadd{r}}\DIFadd{'}\DIFaddend )\DIFdelbegin %DIFDELCMD < \right]\left[%%%
\DIFdelend \DIFaddbegin \big]  \big[\DIFaddend \nabla \DIFdelbegin \DIFdel{u}\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ({\bf r})\DIFdelbegin \DIFdel{^2-T}\DIFdelend \DIFaddbegin \big] \DIFadd{\cdot }\big[\DIFaddend \nabla \DIFdelbegin \DIFdel{^2 u}\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ({\bf r}\DIFaddbegin \DIFadd{'}\DIFaddend )\DIFdelbegin %DIFDELCMD < \right]%%%
\DIFdel{\,}\DIFdelend \DIFaddbegin \big] \DIFadd{\dd{\bf r} \dd{\bf r}' }\DIFaddend .
	\end{aligned}
\end{equation}
\DIFdelbegin \DIFdel{We note the structure of the empirical connection in }%DIFDELCMD < \eqref{eq:empiricalbalance1} %%%
\DIFdelend \DIFaddbegin \DIFadd{Interestingly, it }\DIFaddend is reminiscent of the connection between \DIFdelbegin \DIFdel{two and three body correlations at equilibrium as }\DIFdelend \DIFaddbegin \DIFadd{density correlations }\DIFaddend provided by the YBG hierarchy \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{Hansen2013} }%DIFAUXCMD
with the equilibrium pair correlation function replaced by correlation function averaged over the equilibrium and non-equilibrium steady states.
Finally, we note that this connection can be further rewritten in a compact form that relates the deviations of two and three body correlations from their equilibrium values, 
}\begin{displaymath}
    \DIFdel{\label{eq:empiricalbalance2}
    \begin{aligned}
    }&\DIFdel{\rho^2 \int \int\ d}{\DIFdel{\bf r}} \DIFdel{d}{\DIFdel{\bf r}}\DIFdel{^\prime \left[g_3(}{\DIFdel{\bf r}}\DIFdel{,}{\DIFdel{\bf r}}\DIFdel{^\prime)-g_{3,\rm eq}(}{\DIFdel{\bf r}}\DIFdel{,}{\DIFdel{\bf r}}\DIFdel{^\prime)\right]\left[\nabla u(}{\DIFdel{\bf r}}\DIFdel{)\cdot \nabla u(}{\DIFdel{\bf r}}\DIFdel{^\prime)\right]}\\
    &\DIFdel{\quad =-\rho_0 \int d}{\DIFdel{\bf r}}\DIFdel{\frac{1}{2}\left[g(}{\DIFdel{\bf r}}\DIFdel{-g_{\rm eq}(}{\DIFdel{\bf r}}\DIFdel{)\right]\left[\nabla u(}{\DIFdel{\bf r}}\DIFdel{)^2-T\nabla^2 u(}{\DIFdel{\bf r}}\DIFdel{)\right]\,.
    \end{aligned}
}\end{displaymath}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{at equilibrium~\mbox{%DIFAUXCMD
\cite{Hansen2013}}%DIFAUXCMD
.
}\DIFaddend 
...
\begin{equation}\label{eq:balance_b}
	\begin{aligned}
		\DIFdelbegin \DIFdel{\gamma}\DIFdelend \dot w &= \DIFdelbegin \DIFdel{\rho_0 }\DIFdelend \DIFaddbegin \DIFadd{\frac{\rho_0}{\gamma} }\DIFaddend \int g({\bf r}) \big[(\nabla v({\bf r}))^2 - T \nabla^2 v({\bf r})\big] \dd{\bf r} ,
		\\
		&\quad + \DIFdelbegin \DIFdel{\rho_0^2 }\DIFdelend \DIFaddbegin \DIFadd{\frac{\rho_0^2}{\gamma} }\DIFaddend \iint g_3({\bf r}, {\bf r}') \big[\nabla v({\bf r})\big] \cdot \big[\nabla v({\bf r}')\big] \dd{\bf r} \dd{\bf r}' ,
	\end{aligned}
\end{equation}
...
\begin{equation}\DIFdelbegin \DIFdel{\gamma}\DIFdelend \DIFaddbegin \label{eq:balance_c}
	\DIFaddend \dot w = \DIFaddbegin \DIFadd{\frac{1}{\gamma}}\DIFaddend \sum_i \big[ \big\langle{\bf F}_i^2\big\rangle + T \big\langle\nabla_i\cdot{\bf F}_i\big\rangle \big] ,
\end{equation}
\DIFdelbegin \DIFdel{formally analogue to the relationin}\DIFdelend \DIFaddbegin \DIFadd{which differs from the relation}\DIFaddend ~\eqref{eq:approx} for driven tracers \DIFdelbegin \DIFdel{up to }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend an overall factor of $2$. \DIFdelbegin \DIFdel{Conducting the }\DIFdelend \DIFaddbegin \DIFadd{A result analogue to~}\eqref{eq:balance_c} \DIFadd{was found previously for deterministic drive~\mbox{%DIFAUXCMD
\cite{Maes2010, Maes2015}}%DIFAUXCMD
. The main difference is that~}\eqref{eq:balance_c} \DIFadd{only features interaction forces ${\bf F}_i$ in the rhs, thus allowing one to evaluate the rate of work without any prior knowledge on the driving force. Besides, it is valid for both deterministic and active drives. Moreover, conducting the }\DIFaddend same analysis of density correlations as for driven tracers, ${\cal I}$ exhibits again a scaling with ${\rm Pe}^2$, as reported in Fig.~\ref{fig:fig2_ac}. \DIFdelbegin \DIFdel{Besides, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend show that $\dot w$ and $\int{\cal I}({\bf r}){\rm d}{\bf r}$ are in good agreement when now introducing the factor $\rho_0/(2\gamma)$. \DIFdelbegin \DIFdel{We note that this empirical connection with the modified factor of $\rho_0/(2\gamma)$ is still consistent with the non-equilibrium connection between two and three body correlations proposed in }%DIFDELCMD < \eqref{eq:empiricalbalance1}%%%
\DIFdel{, }%DIFDELCMD < \eqref{eq:empiricalbalance2}%%%
\DIFdel{. These results demonstrate }\DIFdelend \DIFaddbegin \DIFadd{This is consistent with substituting~}\eqref{eq:emp_b} \DIFadd{into~}\eqref{eq:balance_b}\DIFadd{, where $g_{3,a}+g_{3,b}$ is now replaced by $2g_3$, as expected. Hence, it demonstrates }\DIFaddend that dissipation is also accessible from the nonequilibrium deviation of pair \DIFdelbegin \DIFdel{correlation even }\DIFdelend \DIFaddbegin \DIFadd{correlations }\DIFaddend in fully driven \DIFdelbegin \DIFdel{(or active) }\DIFdelend liquids.


Overall, the results of this Section illustrate how dissipation affects the transport and structural properties of driven liquids, measured in terms of diffusion coefficient and density fluctuations. These findings motivate the following question: can nonequilibrium forces be tuned to reliably stabilize target configurations? To explore this, we rely in what follows on the framework of large deviation theory. In practice, our strategy amounts to biasing trajectories in terms of dissipation, related to many-body interactions by~\eqref{eq:balance}, to mimic the effect of an external drive. Following this route, our analytical and numerical results provide some concrete intuition for how interactions in a multicomponent system can be controllably renormalized by nonequilibrium forces. Hence, we demonstrate the ability to nucleate structures different from those characteristic of the equilibrium Boltzmann distribution to help guiding self-assembly~\cite{Bisker2018} and collective motion~\cite{Vicsek1995} far from equilibrium. These results further illustrate the interplay between energy {dissipation} and organization in nonequilibrium many-body settings.
...
\begin{equation}\label{eq:dyn_eq}
	\gamma\dot{\bf r}_i = - \nabla_i \sum_j v({\bf r}_i-{\bf r}_j) + {\boldsymbol\xi}_i ,
\end{equation}
where the statistics of the noise term ${\boldsymbol\xi}_i$ is the same as the one in~\eqref{eq:dyn}. The rate of work $\dot w$ defined in~\eqref{eq:work} is zero because of the absence of \DIFdelbegin \DIFdel{the drivingforce}\DIFdelend \DIFaddbegin \DIFadd{driving}\DIFaddend . In Sec.~\ref{sec:method}, to obtain a non-zero rate of energy flow through the system, we consider an explicit driving force ${\bf F}_{{\rm d},i}$ and we explore its effects on the \DIFaddbegin \DIFadd{transport and }\DIFaddend structural properties of the liquid. In \DIFaddbegin \DIFadd{practice, different types of driving can lead to same dissipation. In }\DIFaddend this section, using the framework of large deviation theory, we take an alternative approach where the dynamics is now conditioned by enforcing a required energy flow \DIFaddbegin \DIFadd{without any explicit driving}\DIFaddend . Thus, exploring how the system adapts to this requirement provides a new insight \DIFdelbegin \DIFdel{, distinct from the one in Sec.~\ref{sec:method}, }\DIFdelend into the relation between dissipation \DIFdelbegin \DIFdel{, structural properties }\DIFdelend and organization in driven systems\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, which is distinct from yet complements the approach in Sec.~\ref{sec:method}.
}\DIFaddend 


To this end, we focus on the subset of noise realizations that are effectively equivalent to dynamics with non-zero rate of work. Such dynamics are {\it atypical} since they correspond to realizations largely deviated from the most probable ones, thus opening the door to stabilizing structure and to promoting collective effects which are not {\it typically} observed. In particular, for this subset, the noise term in~\eqref{eq:dyn_eq} no longer has zero average, so that one can re-define it as \DIFdelbegin \DIFdel{${\boldsymbol \xi}_i \to {\boldsymbol \xi}_i + {\bf F}_i$ }\DIFdelend \DIFaddbegin \DIFadd{${\boldsymbol \xi}_i \to {\boldsymbol \xi}_i + {\bf F}_{{\rm aux},i}$ }\DIFaddend in terms of an {\it auxiliary force} force \DIFdelbegin \DIFdel{${\bf F}_i$}\DIFdelend \DIFaddbegin \DIFadd{${\bf F}_{{\rm aux},i}$}\DIFaddend ~\cite{Jack2010, Chetrite2013}. Hence, \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{the dynamics given by~}\eqref{eq:dyn_eq} \DIFadd{with added force ${\bf F}_{{\rm aux},i}$ }\DIFaddend provides an explicit realization \DIFdelbegin \DIFdel{of forces that will ensure }\DIFdelend \DIFaddbegin \DIFadd{which ensures }\DIFaddend a non-zero energy flow rate.
...
\subsection{\DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend Dynamical bias and external forces\DIFdelbegin %DIFDELCMD < }%%%
...
\end{equation}
In practice, computing $\cal G$ is a highly non-trivial procedure for many-body systems. The explicit solutions \DIFdelbegin \DIFdel{obtained so far for diffusive }\DIFdelend \DIFaddbegin \DIFadd{considered so far concern either exclusion processes~\mbox{%DIFAUXCMD
\cite{Popkov2010, Popkov2011, Limmer2018a} }%DIFAUXCMD
or }\DIFaddend particle-based \DIFdelbegin \DIFdel{dynamics are mostly restricted to }\DIFdelend \DIFaddbegin \DIFadd{diffusive systems restricted to small noise regimes~\mbox{%DIFAUXCMD
\cite{Lecomte2019, Proesmans2019} }%DIFAUXCMD
and }\DIFaddend non-interacting \DIFdelbegin \DIFdel{systems~\mbox{%DIFAUXCMD
\cite{Chetrite2013, Touchette2016}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{cases in some specific potentials~\mbox{%DIFAUXCMD
\cite{Majumdar2002, Touchette2016, Touchette2018}}%DIFAUXCMD
}\DIFaddend .
...
\begin{figure*}
	...
	\end{flushleft}
	\vskip-.6cm
	\DIFdelbeginFL \DIFdelFL{\raisebox{2.3cm}}%DIFDELCMD < {%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\raisebox{2.1cm}}{\DIFaddendFL \rotatebox{90}{\large $D_{\rm r}>D_{\rm r}^*$}}
	...
	\rule{5.81cm}{.2mm}
	\vskip-.05cm
	\DIFdelbeginFL \DIFdelFL{\raisebox{2.35cm}}%DIFDELCMD < {%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\raisebox{2.2cm}}{\DIFaddendFL \rotatebox{90}{\large $D_{\rm r}<D_{\rm r}^*$}}
	\includegraphics[width=.318\linewidth]{snap_k02D7.pdf}
	\DIFdelbeginFL \DIFdelFL{\raisebox{-0.7cm}}%DIFDELCMD < {%%%
\DIFdelFL{\rule{.2mm}{6.5cm}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\raisebox{-.5cm}}{\DIFaddFL{\rule{.2mm}{6.3cm}}\DIFaddendFL }
	\includegraphics[width=.318\linewidth]{snap_k0D7.pdf}
	\includegraphics[width=.318\linewidth]{snap_km02D7.pdf}
	\begin{flushleft}
		\vskip\DIFdelbeginFL \DIFdelFL{-.5}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-.6}\DIFaddendFL cm
		{\large
			\hskip2.55cm
			$\kappa=-0.2$
			\hskip4.48cm
