...
Motivated by these results, and to provide concrete intuition for how particular configurations can be stabilized by nonequilibrium forces, we investigate the emerging structure of Brownian particles subject to dynamical bias. Our aim is to preferentially select trajectories which mimic the effect of an external drive. In Sec.~\ref{sec:bias}, we first put forward a biased ensemble where trajectories are indeed equivalent, in a perturbative regime, to the ones driven by external forces. Then, we identify a class of biases which affects interactions between the particles in a controlled manner: this is our second main result. The explicit form of the bias is inspired by the results of Sec.~\ref{sec:method} connecting dissipation to many-body interactions. Direct sampling of these ensembles, based on cloning algorithm~\cite{Giadina2006, tailleur2007probing, Hurtado2009, Nemoto2016, Ray2018, Klymko2018, Brewer2018}, confirms that our bias regulates the liquid structure in a supervised way, leading to specific spatial organization. In practice, the rare noise fluctuations sampled with dynamical bias effectively drive the system away from typical behavior~\cite{garrahan2007, Hedges2009, Jack2010, Pitard2011, Speck2012, Bodineau2012a, Chetrite2013, Limmer2014, Nemoto2017}. Such noise realizations can then serve as proxies of how to control the dynamics by applying an external force with complex protocols. Overall, our results lay the groundwork for precise control of the emerging structure in many-body diffusive nonequilibrium systems. In practice, it could potentially result in design principles for addressable self-assembly where, for instance, specific interactions are tailored to stabilize target molecules.
...
Our first goal is to show that some biased ensembles are effectively equivalent to an ensemble driven by an external force. Inspired by the role of dissipation in emerging liquid properties discussed in Sec.~\ref{sec:method}, we consider biasing the equilibrium dynamics~\eqref{eq:dyn_eq} with the {\it virtual} rate of work, scaled by $T$, that would be produced by applying a constant force ${\bf F}_{\rm d}$ to a subset $\Omega$ of particles:
\begin{equation}\label{eq:eps}
	{\cal E} = \frac{1}{\gamma T}\sum_{i\in\Omega} {\bf F}_{\rm d}\cdot \nabla_i V ,
\end{equation}
where $V=(1/2)\sum_{i,j}v({\bf r}_i-{\bf r}_j)$. Following the procedure in~\cite{Jack2010,Chetrite2013}, an auxiliary physical dynamics which has the same statistical properties as in the biased ensemble can be constructed by solving the following eigenvalue equation 
...
\end{equation}
Expanding~\eqref{eq:action} and eliminating a boundary term, which is negligible at large times provided that particles are confined in a finite volume, we get
...
\end{equation}
From inspection of~\eqref{eq:action_exp}, it appears that $\cal P$ exactly coincides with the path probability of the original dynamics~\eqref{eq:dyn_eq} biased with~\eqref{eq:eps}. Therefore, the original biased dynamics is actually identical, at any order, to the combination of introducing an external force in the dynamics and biasing with a constant term symmetric in $\kappa$, as described respectively in~\eqref{eq:V} and~\eqref{eq:eps_prime}.
