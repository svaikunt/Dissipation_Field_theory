...
Then, we identify a class of biasing observables which all affect interactions in a controlled manner: this is our second main result. As a proof of principle, we analyze biased trajectories for a specific choice in this class where, at any order, biasing can be simply rationalized as enhanced clustering with renormalized interactions. Direct sampling of such a biased ensemble, based on cloning algorithm~\cite{Giadina2006, tailleur2007probing, Hurtado2009, Nemoto2016, Ray2018, Klymko2018, Brewer2018}, confirms that tuning desired interactions regulates the liquid structure in a supervised way, leading to specific spatial organization. Overall, this lays the ground for precise control of the emerging structure in many-body diffusive systems. In practice, it could potentially result in design principles for addressable self-assembly, for instance, by tailoring pair-specific interactions to construct complex energetic landscapes.
...
In this Section, we provide a series of relations between energy dissipation and liquid properties in driven liquids. We first consider the specific periodic drive~\eqref{eq:theta}, as a minimal case study, to show that diffusion of tracer particles can be expressed in terms of dissipation. Then, we explore how density correlations are related to dissipation in more general settings.
...
We then consider the effect of higher-order bias. It can be anticipated from the auxiliary physical dynamics, given by the first-order force~\eqref{eq:V}, biased with
...
\end{equation}
up to a boundary term which is negligible at large times ($t\to\infty$). Hence, it appears from~\eqref{eq:action_exp} that $\cal P$ coincides with the path probability of the original dynamics~\eqref{eq:dyn_eq} biased with~\eqref{eq:eps}. Therefore, the effect of such a bias is actually identical, at any order, to introducing an external force in the dynamics and biasing with a constant term symmetric in $\kappa$, as described respectively in~\eqref{eq:V} and~\eqref{eq:eps_prime}.
...
To go beyond the case of applying a constant force, we now seek for a proper dynamical bias to regulate particle interactions in a controlled manner. To this aim, we examine cases where the control parameters $\kappa_{ij}$ are specific for any particle pairs $\{i,j\}$, so that the biasing weight in path probability now reads $\exp\big[\sum_{i,j}\kappa_{ij}\int_0^t{\cal E}_{ij}(s){\rm d}s\big]$. We first observe that the power balance~\eqref{eq:balance} can be written as $\dot w = - \sum_{i\in\Omega,j\not\in\Omega} \langle{\cal L} v({\bf r}_i-{\bf r}_j)\rangle$, which provides another direct connection between rate of work and many-body interactions in driven liquids. This motivates us to consider the following biasing observable
\begin{equation}\label{eq:eps_bis}
	{\cal E}_{ij} = \frac{1}{T} {\cal L} v({\bf r}_i-{\bf r}_j) .
\end{equation}
With this choice, the property $T\langle{\cal E}_{ij}\rangle = \langle\dot v({\bf r}_i-{\bf r}_j)\rangle = 0$ follows directly, which allows one to derive the first-order auxiliary dynamics as previously. Biasing the equilibrium dynamics~\eqref{eq:dyn_eq} with~\eqref{eq:eps_bis} is now equivalent to a dynamical bias on the first-order auxiliary dynamics, given by the potential $(1/2)\sum_{i,j}(1+2\kappa_{ij})v({\bf r}_i-{\bf r}_j)$, with respect to the symmetric bias
...
In this Appendix, we demonstrate the equivalence between two dynamical biased ensembles. Ensemble~(a) corresponds to the equilibrium dynamics~\eqref{eq:dyn_eq} biased with $\sum_{i,j} \kappa_{ij}\int_0^t{\cal E}_{ij}(s){\rm d}s$, where
\begin{equation}
