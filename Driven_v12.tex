...
%We show that such biasing leads to a renormalization of interparticle interactions. In this manner, atypical structures can be stabilized in a controlled fashion. We illustrate this with numerical simulations of a minimal case study where sampling is biased with a cloning algorithm. 
...
%where, for instance, specific interactions are tailored to stabilize target molecules. 

% ===============================================================================

%Our aim is to preferentially select trajectories which mimic the effect of an external drive. In Sec.~\ref{sec:bias}, we first put forward a biased ensemble where trajectories are indeed equivalent, in a perturbative regime, to the ones driven by external forces. Then, we identify a class of biases which affects interactions between the particles in a controlled manner: this is our second main result. The explicit form of the bias is inspired by the results of Sec.~\ref{sec:method} connecting dissipation to many-body interactions. Direct sampling of these ensembles, based on cloning algorith, confirms that our bias regulates the liquid structure in a supervised way, leading to specific spatial organization. In practice, the rare noise fluctuations sampled with dynamical bias effectively drive the system away from typical behavior~\cite{garrahan2007, Hedges2009, Jack2010, Pitard2011, Speck2012, Bodineau2012a, Chetrite2013, Limmer2014, Nemoto2017}. Such noise realizations can then serve as proxies of how to control the dynamics by applying an external force with complex protocols.
...
%immediately suggests that internal transport and emerging structure can potentially be tuned using the external nonequilibrium forces which power the dissipation. 
...
%$\dot{w}=-\sum_{i\in\Omega,j\not\in\Omega}1\langle {\cal E}_{ij}\rangle $. This connection motivates the bond dissipation rate definition above. 

%We then define the dissipation by these bonds, or the rate at which energy is pumped into or extracted out of the pairwise interactions by the thermal noise, as 
%\begin{equation}\label{eq:eps_ij}
%{\cal E}_{ij}=\frac{1}{\gamma T}\sum_{k,\ell}\left %(\delta_{k,i}\delta_{\ell,j} + \delta_{k,j}\delta_{\ell,i}\right ) %{\boldsymbol \xi}_k \cdot\nabla_k v ({\bf r}_k-{\bf r}_\ell),    
%\end{equation}
%where $\delta_{i,j}$ is the Kronecker delta. Indeed,
%caused by the thermal noise
%such effective interactions only affect the targeted bond between
%This means that tuning the desired interactions between particles, which is often difficult in experiments, is possible without modifying the dynamical equations, by constructing effective interactions in the use of atypical thermal noise realizations

% T.N.: Commented out from #1
%${\cal E}_{ij}=\frac{1}{\gamma T}\sum_k\big[ T \nabla_k - \nabla_k V\big] \cdot\nabla_i v ({\bf r}_i-{\bf r}_j)$ where $V=(1/2)\sum_{i,j}v({\bf r}_i-{\bf r}_j)$. From Eq.~\ref{eq:balance}, we see that $\dot{w}=-\sum_{i\in\Omega,j\not\in\Omega}1\langle {\cal E}_{ij}\rangle $. In other words, $\langle {\cal E}_{ij}\rangle $ captures the rate at which energy is pumped into or extracted out of the pairwise interactions between the $i^{\rm th}$ and $j^{\rm th}$ particles. By importance sampling trajectories based on the statistics of ${\cal E}_{ij}$, in the next section we show that the interactions between particles can be renormalized in a controlled manner. The biased particle trajectories are meant to mimic the effect of an external drive. In a multicomponent fluid, the controlled renormalization of interactions at the cost of biasing ${\cal E}_{ij}$ can promote the nucleation of structures different from those charecteristic of the equilibrium Boltzmann distribution. These results further illustrate the interplay between energy consumption and organization in many body non-equilibrium settings.}


%A related question then is whether specific configurations and correlations could be stabilized alternatively by biasing particle trajectories which mimic the effect of an external drive. We now investigate this question. 
...
%\textcolor{blue}{large deviation biased ensembles are generated by} 
...
%where $\cal E$ and $\kappa$ respectively denote a given observable and its associated control parameter. The trajectories and configurations in the biased ensemble can then be regarded as the large-deviation realizations conditioned by atypical values of $\cal E$, which can be tuned at will by changing $\kappa$.
...
%to use the thermal noise as a probe
\tn{To pedagogically introduce our idea,} our first goal is to show %that some biased ensembles are effectively equivalent to an ensemble driven by an external force. 
\tn{how the bias in the dissipation can lead to an effective driving force.}
Inspired by the role of dissipation in emerging liquid properties discussed in Sec.~\ref{sec:method}, we consider as the biasing function %biasing the equilibrium dynamics~\eqref{eq:dyn_eq} with 
the {\it \tn{virtual excess}} rate of work, scaled by $T$, that would be produced by applying a constant force ${\bf F}_{\rm d}$ to a subset $\Omega$ of %\tn{the noise term}
particles:
%\begin{equation}\label{eq:eps}%
%	{\cal E} = \frac{1}{\gamma T}\sum_{i\in\Omega} {\bf F}_{\rm d}\cdot \nabla_i V ,
%\end{equation
%where $V=(1/2)\sum_{i,j}v({\bf r}_i-{\bf r}_j)$.
%\tn{
%\begin{equation}\label{eq:eps}%
%	{\cal E} = \frac{1}{\gamma T}\sum_{i\in\Omega}{\boldsymbol \xi}_i \cdot {\bf F}_{\rm d}.
%\end{equation}}
\tn{
\begin{equation}\label{eq:eps}%
...
%\tn{Our purpose is to find a bias that is equivalent to a forced system by an auxiliary force. 
%
%}
...
Following the procedure in~\cite{Jack2010,Chetrite2013}, an auxiliary physical dynamics %which has the same statistical properties as in the biased ensemble 
...
\end{equation}
%\tn{ The parameter $\kappa$ is connected to the conditioned value $\cal E^{\prime}$ via Legendre transform of this scaled cumulant generating function: $\partial \lambda/\partial \kappa = \cal E^{\prime}$. Note that when ${\cal}E^{\prime}=0$, $\kappa$ is also zero.}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
where ${\cal G}_0$ is the uniform eigenvector associated with the zero eigenvalue. Given that $\langle{\cal E}\rangle=0$ \tn{in unbiased dynamics},  %steady state, which follows from the vanishing current condition in the unbiased dynamics ($\langle\dot{\bf r}_i\rangle=0$), 
...
%$T{\cal G}_1 = {\cal G}_0 \sum_{i\in\Omega}{\bf F}_{\rm d}\cdot{\bf r}_i$ is a solution of the eigenvalue problem to order $\kappa$ \footnote{A confining potential at the boundary is assumed to derive this solution, though it is straightforward to extend the same argument to the case without such a potential.}. 
...
\end{equation}
Expanding~\eqref{eq:action}, %and eliminating a boundary term, which is negligible at large times provided that particles are confined in a finite volume, 
we get
\tn{
...
%Therefore, the original biased dynamics is  identical, at any order, to the combination of introducing an external force in the dynamics and biasing with a constant term symmetric in $\kappa$, as described respectively in~\eqref{eq:V} and~\eqref{eq:eps_prime}.
\tn{By noticing
...
%Specifically, we observe that the power balance~\eqref{eq:balance} can be written as $\dot w = - \sum_{i\in\Omega,j\not\in\Omega} \langle{\cal L} v({\bf r}_i-{\bf r}_j)\rangle$. This motivates us to consider the following bias
